...
export default function SearchPage() {
  ...
  const empty = results && results.folders.length === 0 && results.lessons.length === 0;

  return (
    <div className="min-h-screen pb-16">
      <Breadcrumbs
        trail={[{ href: "/home", label: "Home" }, { href: "/search", label: "Search" }]}
        isOwner={!!user}
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6">
        <form onSubmit={runSearch} className="flex gap-2 mb-6">
          <input
            ...
            placeholder="Search folders and lessons…"
...
function ResultRow({ label, sub, onClick }) {
  return (
    <button onClick={onClick} className="w-full text-left bg-white rounded-2xl shadow-sm px-4 py-3 text-sm">
